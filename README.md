...
Runs [Go](http://golang.org) on [OpenShift](https://openshift.redhat.com/app/login) using downloadable cartridge support.  To install to OpenShift from the CLI (you'll need version 1.9 or later of rhc), run:
    ...
    rhc create-app mygo https://cartreflect-claytondev.rhcloud.com/reflect?github=snocorp/openshift-go-cart
...
The repository contains a sample go file which will print "hello, world" when someone hits your web application - see [web.go](https://github.com/snocorp/openshift-go-cart/blob/master/template/web.go).
...
When you push code to your repo, a Git postreceive hook runs and invokes the `bin/compile` script.  This attempts to download a Go environment for you into `$OPENSHIFT_GO_DIR/cache`. The dependencies are expected to be vendored in `./vendor`.  The main file that you run will have access to two environment variables, `$OPENSHIFT_GO_IP` and `$OPENSHIFT_GO_PORT`, which contain the internal address you must listen on to receive HTTP requests to your application.
